...
If no event ID is given, the event ID (and thus the [`lastId` property](#sessionlastid-string)) is set to a unique string generated using a [cryptographic pseudorandom number generator](https://nodejs.org/api/crypto.html#cryptorandomuuidoptions).
...
<Aside type="caution" title="DEPRECATED">
    This method is deprecated and should no longer be used. [See here](https://github.com/MatthewWid/better-sse/issues/52) for more information.
</Aside>
...
### `createResponse`: `<State>(...args: ConstructorParameters<typeof Session>, (session: Session<State>) => void) => Response`

A convenience function to create a new session and return its `Response` immediately.

The last argument should be a callback function that will be invoked with an instance of Session once it has connected.
